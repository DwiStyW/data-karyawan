...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // pendidikan
        $pend=DB::select("SELECT id_master,tanggal_lahir from pendidikan join master on master.id=pendidikan.id_master where tingkat_pendidikan!='Lainnya' group by id_master,tanggal_lahir");
        if(count($pend)!=0){
            foreach($pend as $p){
                $idm=$p->id_master;
