...
<body>
    ...
    <main class="wrapper">
        <div class="container-custome">
            ...
            <div class="p-3">
                @foreach ($pengajuan as $find)
                    <form action="/pengajuan/update/{{ $id }}" method="POST">
                        @csrf
                            ...
                            <div class="col-md-9 col-8">
                                <select name="pendidikan" id="pendidikan">
                                    ...
                                    @foreach ($pendidikan as $p)
                                        @if ($p->tingkatan_pendidikan == $find->pendidikan_terakhir)
                                            <option value="{{ $p->tingkatan_pendidikan }}" selected>
                                                {{ $p->tingkatan_pendidikan }}
                                            </option>
                                        @else
                                            <option value="{{ $p->tingkatan_pendidikan }}">
                                                {{ $p->tingkatan_pendidikan }}
                                            </option>
                                        @endif
                                    @endforeach
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-9 col-8">
                                <input class="form-control" type="number" name="max_usia" id="max_usia"
                                    placeholder="0" value="{{ $find->max_usia }}">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-3 col-4">
...
<script>
    $(document).ready(function() {
        ...
        var $selectprofesi = $('#profesi').selectize();
        var selectizep = $selectprofesi[0].selectize;
        // selectize.setValue('Apoteker');
        selectizep.setValue(@json($find->profesi));

        var $selectjk = $('#jenis_kelamin').selectize();
        var selectizejk = $selectjk[0].selectize;
        // selectizejk.setValue('L');
        selectizejk.setValue(@json($find->jenis_kelamin));
    });
</script>
